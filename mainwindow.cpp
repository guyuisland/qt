...
void MainWindow::on_creatTravelPath_Click_clicked()
{
    ...
    if(ui->Mode_Select->currentIndex() == 0)
        path = client.least_money(the_money);
    if(ui->Mode_Select->currentIndex() == 1)
        path = client.least_time(the_time, the_money);
...
void MainWindow::printout(vector<imfo> path)
{
    ...
    vector<imfo>::iterator iter1;
        ...
        str+=("总时间为："+QString::number(the_time) + "小时\n");
        str+=("总费用为："+QString::number(the_money) + "元");
